...
export default class StorageManager implements StorageManagerInterface {
    ...
    constructor({
        ...
        this.registry = new StorageRegistry({
            fieldTypes: fieldTypes || createDefaultFieldTypeRegistry(),
        })
    ...
    collection(collectionName: string): StorageCollection {
        const operation =
            (operationName) =>
            (...args) =>
                this.operation(operationName, collectionName, ...args)
        return fromPairs([
            ...Array.from(COLLECTION_OPERATIONS).map((operationName) => [
                operationName,
    ...
    async operation(operationName: string, ...args) {
        let next = {
            process: ({
                ...
                | Omit<StorageMiddlewareContext, 'next'>) =>
                this.backend.operation(operation[0], ...operation.slice(1)),
        }

        let extraData = {}
        for (const middleware of this._middleware) {
            next = ((currentNext, currentMiddleware: StorageMiddleware) => ({
                process: async (args) => {
                    extraData = { ...extraData, ...(args.extraData || {}) }
                    const result = await currentMiddleware.process({
                        ...args,
                        extraData,
                        next: currentNext,
                    })
