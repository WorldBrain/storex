...
export function testStorageBackendOperations(backendCreator: StorexBackendTestBackendCreator) {
    ...
    describe('creating and finding', () => {
        ...
        it('should be able to create objects and find them again by string field', async function (context: TestContext) {
            const { storageManager } = await setupUserAdminTest({ context })
            const { object } = await storageManager.collection('user').createObject({ identifier: 'email:joe@doe.com', isActive: true })
            expect(object.id).not.toBe(undefined)
        ...
        it('should be able to create objects and find them again by boolean field', async function (context: TestContext) {
            const { storageManager } = await setupUserAdminTest({ context })
            const { object } = await storageManager.collection('user').createObject({ identifier: 'email:joe@doe.com', isActive: true })
            expect(object.id).not.toBe(undefined)
    ...
    describe('batching', () => {
        ...
        it('should ignore empty batch operations', { shouldSupport: ['executeBatch'] }, async function (context: TestContext) {
            const { storageManager } = await setupChildOfTest({ backend: context.backend })
